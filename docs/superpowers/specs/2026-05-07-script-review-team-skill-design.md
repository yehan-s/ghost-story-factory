...
### 1.1 项目现状
- 现有剧本「杭州夜班保安」v7,145 节点 / ~5 万字 / 7 地标 / 13 共享 NPC / 8 主结局 / 21 种结局体验
...
### 1.2 用户痛点
1. **剧情悖论多** — 时序矛盾、因果漏洞、角色立场漂移在现有 145 节点里没有统一守门人
...
| 2 | **State Architect**<br>状态系统建筑师 | **Linus 风格的反加法者**(见 3.3)——设计玩家知识图谱 + flags + variants 条件矩阵,从根上消除"重复操作给同样反馈" | 状态体系设计、引擎扩展需求清单 |
...
### 3.3 为什么 State Architect 被指定为"Linus 风格的反加法者"
其他 6 个角色都倾向"加内容"(加伏笔、加周目、加 lore、加可视化、加测试)。如果不指定一个反加法者,7 人会集体推动状态空间膨胀,直到不可维护。

State Architect 的工作模式:
- 默认对每个新增 flag/state 持反对态度,要求提案者证明"必须全局",否则降级为局部 visit_count 或就地 narrative_variants
- 与 Topology Designer 配对工作:State 管"状态怎么命名",Topology 管"状态有几个维度"
- 这是有意识的角色配置,不是描述风格,故在表格中保留
...
3. skill 用 TeamCreate 启动 7 人 agent team
   ↓
4. 助手把任务描述发给 Chief Editor(单点入口,不广播)
   ↓
...
8. 7 人并行产出意见(放行者一句话即可)
   ↓
9. Chief Editor 汇总意见 + 同步更新 INDEX.md → 写入 docs/team-reviews/YYYY-MM-DD-<slug>.md
   ↓
10. 助手把报告路径返给用户,等用户审阅
   ↓
11a. 用户放行 → 进入开发(Edit/Write 或 writing-plans)
11b. 用户打回 → 重新评审或要求团队补充
```

**关键顺序约束**:
- 第 5 步(层级判定)必须在第 7 步(相关度自报)之前——这样自报才有依据,不会变成 7 人各说各话
...
### 9.1 试金石任务的验收 checklist
执行第 9 项后,必须满足:
- [ ] 评审报告产出在 `docs/team-reviews/` 下
- [ ] 报告 9 节齐全,每节都有具名角色
- [ ] Chief Editor 能识别出"对讲机问题不是引擎缺陷,是数据层 variants 覆盖不足"这个根本判断(本 spec § 1.2 已明确)
- [ ] State Architect 给出"如何写新 variants 才能消除特殊情况"的具体建议
- [ ] Topology Designer 至少质问一次"这个新增 flag 是否必要"
- [ ] QA 给出至少一个可重复的测试路径
- [ ] INDEX.md 更新,包含此次评审记录
- [ ] 整个流程在 30 分钟内完成(超过则说明 skill 流程过重,需简化)
