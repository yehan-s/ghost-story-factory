...
PROFILE_INHERIT_JSON="$(mktemp -t profile_inherit.XXXXXX.json)"
trap 'rm -f "$CROSS_RUN_JSON" "$VARIANT_TRIGGER_JSON" "$PROFILE_INHERIT_JSON"' EXIT

echo "=== 1/13 audit_playability(GameTree 可玩闭环)==="
python tools/audit_playability.py "$TREE"
echo
echo "=== 2/13 audit_sandbox(ADR-010 沙盒骨架)==="
python tools/audit_sandbox.py "$TREE"
echo
echo "=== 3/13 audit_script_depth(Pass 9 剧本厚度)==="
python tools/audit_script_depth.py "$TREE"
echo
echo "=== 4/13 audit_tree(节点引用完整性 + lore 红线)==="
python tools/audit_tree.py "$TREE"
echo
echo "=== 5/13 audit_state(flags / inv 引用矩阵)==="
python tools/audit_state.py "$TREE"
echo
echo "=== 6/13 audit_variants(variant 覆盖矩阵 + 重访无分化)==="
python tools/audit_variants.py "$TREE"
echo
echo "=== 7/13 audit_reactions(反应式 variant 三红线 ADR-008)==="
python tools/audit_reactions.py "$TREE" > /dev/null
echo "  反应契约审计通过"
echo
echo "=== 8/13 audit_paths_linmou(linmou 必死不变量 ADR-009)==="
python tools/audit_paths_linmou.py "$TREE" > /dev/null
echo "  必死不变量审计通过"
echo
echo "=== 9/13 audit_foreshadow_chain(Pass 22 — 伏笔链条完整性)==="
python tools/audit_foreshadow_chain.py "$TREE" > /dev/null
echo "  伏笔链条审计通过"
echo
echo "=== 10/13 audit_cross_run_continuity(Pass 22 — 跨周目消费侧覆盖)==="
...
echo
echo "=== 11/13 audit_variant_trigger(Pass 22 — variant 触发难度)==="
...
echo
echo "=== 12/13 audit_protagonist_behavior(Pass 22 — G-273 行为画像不变量)==="
